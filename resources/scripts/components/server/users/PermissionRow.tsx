...
const Container = styled.label`
    ${tw`
        flex items-center rounded-lg md:p-3 transition-all duration-200
    ...
    &:not(.disabled) {
        ...
        &:hover {
            ${tw`
                border-blue-400/40 bg-[rgba(59,130,246,0.1)] 
        ...
        <Container htmlFor={`permission_${permission}`} className={disabled ? 'disabled' : undefined}>
            <div css={tw`p-2`}>
                <Checkbox
                    ...
                    value={permission}
                    css={tw`
                        w-5 h-5 mr-2 rounded-md border border-neutral-600 
            ...
            <div css={tw`flex-1`}>
                <Label as={'p'} css={tw`font-medium text-neutral-100`}>
                    {pkey}
                </Label>
