        ...
        <div className={'space-y-2 mt-6'}>
            {uploads.map(([name, file]) => (
                <div
                    key={name}
                    className={
                        'flex items-center space-x-3 p-3 rounded-xl border border-[rgba(255,255,255,0.08)] backdrop-blur-md bg-[rgba(255,255,255,0.06)] shadow-md transition-all duration-300 hover:border-cyan-400/40'
                    }
                >
                    ...
                    <Code className={'flex-1 truncate text-cyan-200 font-semibold'}>{name}</Code>
                    <button
                        onClick={cancelFileUpload.bind(this, name)}
                        className={
                            'text-gray-400 hover:text-white transition-colors duration-200 rounded-full p-1 hover:bg-[rgba(255,255,255,0.08)]'
                        }
                    >
            ...
            <Dialog.Footer>
                <Button.Danger
                    ...
                    className="rounded-lg border border-red-400/40 bg-[rgba(255,255,255,0.08)] backdrop-blur-md text-red-300 hover:border-red-300 hover:text-white transition-all duration-300"
                    onClick={() => clearFileUploads()}
                >
                    Cancel Uploads
                </Button.Danger>
                <Button.Text
                    className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.05)] backdrop-blur-md text-gray-300 hover:text-white transition-all duration-300"
                    onClick={close}
                >
                    Close
                </Button.Text>
        ...
        <>
            {count > 0 && (
                <Tooltip content={`${count} files are uploading, click to view`}>
                    <button
                        className={
                            'flex items-center justify-center w-10 h-10 rounded-full border border-[rgba(255,255,255,0.08)] backdrop-blur-md bg-[rgba(255,255,255,0.06)] shadow-md hover:border-cyan-400/40 transition-all duration-300'
                        }
                        onClick={() => (open.value = true)}
                    >
                        <Spinner progress={(progress.uploaded / progress.total) * 100} className={'w-8 h-8'} />
                        <CloudUploadIcon className={'h-3 absolute mx-auto animate-pulse text-cyan-300'} />
                    </button>
                </Tooltip>
            )}
